...
**Common Issue**: SealedSecret decryption failures due to keypair mismatches.

```bash
# 1. Check CSI pods status
kubectl get pods -n csi-proxmox

# 2. Check SealedSecret health
...
# 4. If SealedSecret shows decryption error, regenerate with stable keypair:
...
kubectl get secret sealed-secrets-key -n kube-system -o jsonpath='{.data.tls\.crt}' | base64 -d | openssl x509 -text -noout | grep -A2 "Serial Number"
